...
parse grid = asd initial
  where
    initial = (grid, [0 .. length grid], [0 .. (length $ grid !! 0)], [])
...
asd state@(grid, _, _, _) = go state 0 0
  where
    ...
    go current row col
      | (row == height) && (col == width) = current
      | col == width = go (step current row col) (row + 1) 0
...
step (grid, emptyColumns, emptyRows, galaxies) row col = (grid, emptyColumns', emptyRows', galaxies')
  where
    ...
    emptyRows' = if value == '#' then remove col emptyRows else emptyRows
